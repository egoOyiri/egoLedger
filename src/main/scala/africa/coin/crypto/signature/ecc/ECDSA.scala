package africa.coin.crypto.signature.ecc
...
object ECDSA {
  ...
  def packPublic64(key: Key) =
    Base64.encodeBytes(packPoint(key.pub, key.curve).bigInteger.toByteArray())
  ...
  def hash64(text: String, digest: String = defaultDigest): String =
    Base64.encodeBytes(hash(text, digest).bigInteger.toByteArray())
 ...
 */
  ...
  def sign(key: Key, text: String, digest: String = defaultDigest): String = {
    ...
    Base64.encodeBytes(((r << key.curve.bits) + s).bigInteger.toByteArray());
  }
